...
def main() -> int:
    ...
    print(
        f"point-in-time ever-members {SAMPLE_START}..{END}: {len(ever)} "
    ...
    for i in range(0, len(todo), chunk):
        ...
        print(
            f"  {done}/{len(todo)} attempted, {len(got)} with data, "
    ...
    coverage["removed_coverage"] = coverage["removed_with_data"] / coverage["removed_members"]
    by_year = {}
    ...
    atomic_write_bytes(
        Path(cfg.paths.artifacts_dir) / "universe_coverage.json",
        json.dumps(coverage, indent=2).encode(),
    )
    catalog.audit("pit_universe_ingest", reason="phase1", ever=len(ever), with_data=len(have))

    print("\n=== COVERAGE (the survivorship bias, measured) ===")
    print(
        f"ever-members with usable data: {coverage['with_data']}/{len(ever)} "
        f"({coverage['overall_coverage']:.0%})"
    )
    print(
        f"  still-listed members: {coverage['alive_with_data']}/{len(alive)} "
        f"({coverage['alive_with_data'] / len(alive):.0%})"
    )
    print(
        f"  REMOVED members:      {coverage['removed_with_data']}/{len(dead)} "
        f"({coverage['removed_coverage']:.0%})  <- the critical number"
    )
    for year, row in by_year.items():
        print(
            f"  {year}: {row['with_data']}/{row['members']} PIT members covered "
            f"({row['coverage']:.0%})"
        )
    return 0
