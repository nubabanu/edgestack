...
def main() -> int:
    ...
    pre = merged.loc[(merged["date"] < FREEZE) & merged["gross_ret"].notna()].reset_index(drop=True)
    features_pre = pre.drop(columns=["label_end", "gross_ret", "close", "volume"])
    labels_frame = pre[["symbol", "date", "label_end", "gross_ret"]].assign(horizon=HORIZON)
    experiment_id = catalog.record_experiment("live_frozen_edges", end_date=FREEZE.date())
    ...
    print(f"frozen (pre-2024) validated long edges: {len(validated)} ({time.time() - t0:.0f}s)")
    ...
    frozen_binner = QuantileBinner(quantiles=cfg.discovery.quantile_bins).fit(pre, binnable)
    ...
    regime = "UP" if bench_trend > 0.02 else "DOWN" if bench_trend < -0.02 else "SIDEWAYS"
    ...
    for e in validated:
        match_matrix[e.identity.edge_id] = evaluate_condition(
            e.identity.condition, today, frozen_binner
        ).to_numpy()
    ...
    for i in range(len(today)):
        ...
        if agg is None:
            continue
        r = today.iloc[i]
        prob = float(np.mean([e.stats.probability_of_positive_net_return for e in matched]))
        vol_ann = float(r.get("realized_vol_20", np.nan))
        per_trade_vol = vol_ann / np.sqrt(252) * np.sqrt(HORIZON) if np.isfinite(vol_ann) else 0.05
        ...
        rows.append(
            {
                ...
                "E[net]_10d": agg.expected_net_return,
                "hit": prob,
                "n_edges": len(matched),
                "families": len({e.identity.family for e in matched}),
                "top_edges": "; ".join(e.identity.name.split(": ", 1)[1] for e in matched[:2]),
                "stop": float(r["close"] - 2 * atr),
                "target": float(r["close"] + 2.5 * atr),
            }
        )

    board = pd.DataFrame(rows).sort_values(["conviction", "E[net]_10d"], ascending=False)
    print(
    ...
    payload = {
        ...
        "regime": {
            "bench_trend_200": bench_trend,
            "bench_vol_20": bench_vol,
            "trend": regime,
            "vol": vol_state,
        },
    ...
    atomic_write_bytes(
        Path("artifacts") / "live_board.json", json.dumps(payload, indent=1).encode()
    )
    ...
    weak = today.loc[
        (
            ...
            | ((today["vol_ratio_5_60"] > 1.5) & (abs(bench_trend) <= 0.02) & (bench_vol > 0.2))
        ),
    ...
    print(
        ...
        if len(weak)
        else "  (no symbols in the weak cohorts today)"
    )
