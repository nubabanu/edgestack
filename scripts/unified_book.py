...
SPLITS = {"dev": ("2005-01-01", "2015-12-31"),
...
def main() -> int:
    ...
    def evaluate(book: pd.Series) -> dict:
        ...
        for k, (lo, hi) in SPLITS.items():
            ...
            row[k] = {"sharpe": round(sharpe_ratio(seg.to_numpy()), 2),
                      "cagr": round(float(np.prod(1 + seg))
                                    ** (252 / len(seg)) - 1, 4),
    ...
    results = {}
    print(f"{'combo':<26}{'dev':>7}{'val':>7}{'hold':>7}{'holdDD':>8}"
          f"{'a/yr':>7}{'t':>6}{'beta':>6}")
    for name, series in (("SPY buy&hold", spy), ("core alone", core),
                         ("satellite alone", sat)):
        r = evaluate(series)
        results[name] = r
        print(f"{name:<26}{r['dev']['sharpe']:>7.2f}{r['val']['sharpe']:>7.2f}"
    ...
    for split in (0.8, 0.7, 0.6):
        ...
        for tgt in (None, 0.08, 0.10):
            ...
            r["devval_sharpe"] = round(
                sharpe_ratio(book.loc["2005-01-01":"2023-12-31"]
                             .dropna().to_numpy()), 3)
            grid[label] = r
            results[label] = r
            print(f"{label:<26}{r['dev']['sharpe']:>7.2f}{r['val']['sharpe']:>7.2f}"
                  f"{r['hold']['sharpe']:>7.2f}{r['hold']['maxdd']:>8.1%}"
                  f"{r['alpha_ann']:>7.1%}{r['alpha_t']:>6.2f}{r['beta']:>6.2f}")
    ...
    c = grid[chosen]
    print(f"holdout (untouched by selection): Sharpe {c['hold']['sharpe']}, "
          f"CAGR {c['hold']['cagr']:+.1%}, maxDD {c['hold']['maxdd']:.1%}  |  "
          f"pooled alpha {c['alpha_ann']:+.1%}/yr t={c['alpha_t']}")
    results["SELECTED"] = chosen
    atomic_write_bytes(Path("artifacts") / "unified_book.json",
                       json.dumps(results, indent=1).encode())
