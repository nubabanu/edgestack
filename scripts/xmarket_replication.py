...
SPLITS = {
    "dev_pre2016": ("1996-01-01", "2015-12-31"),
...
def main() -> int:
    ...
    for sym, name in MARKETS.items():
        ...
        row = {"symbol": sym, "market": name, "since": str(df.index[0].date())}
        passes = 0
        ...
        tde = pd.Series(df.groupby(ym).cumcount(ascending=False) + 1, index=df.index)
        tom = (tdom <= 3) | (tde == 1)
    ...
    print(
        f"{'mkt':<5}{'since':<12}{'dev':>11}{'val':>11}{'hold':>11}"
    ...
    atomic_write_bytes(
        Path("artifacts") / "xmarket_replication.json", json.dumps(rows, indent=1).encode()
    )
