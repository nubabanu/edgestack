...
VETOES = ("skip if SPY < 200-DMA", "skip if 20d vol > 20%",
          "never leverage", "size = pain limit / worst-case %")


def main() -> int:
    today = date.today()
    end = today + timedelta(days=400)
    cal = xcals.get_calendar("XNYS", start=str(today - timedelta(days=40)),
                             end=str(end))
    ...
    for y, m in months:
        ...
        if m == 11:
            ...
            events.append({
                "date": str(tue.date()), "trade": "THANKSGIVING WED",
                "action": "buy SPY at the close; sell Wednesday close",
                "hit": "70% (SPY) / 81% (QQQ, fatter tail)", "worst": "-2.4% (SPY)",
                "note": "do not initiate anything new before the 4-day closure"})
            if len(ms) >= 7:
                events.append({
                    "date": str(ms[5].date()), "trade": "NOV WORST-DAY FLAT",
        ...
        if m == 12 and len(ms) >= 15:
            events.append({
                "date": str(ms[13].date()), "trade": "PRE-CHRISTMAS",
                "action": "buy SPY at the close; sell next close (td15)",
                "hit": "73%", "worst": "small",
                "note": "optional; Santa window is milder (+12bps/day)"})

    events.sort(key=lambda e: e["date"])
    payload = {"generated_for": str(today), "vetoes": list(VETOES),
               "event_trigger": {
                   "name": "RED CLOSE IN CALM UPTREND (any day)",
                   "rule": "SPY down on the day, above 200-DMA, vol<20% -> "
                           "buy the close, sell next open",
                   "hit": "60%", "worst_p5": "-0.8%",
                   "stat": "t=5.41, the gentlest trade found"},
               "events": events}
    atomic_write_bytes(Path("artifacts") / "sniper_calendar.json",
                       json.dumps(payload, indent=1).encode())
    print(f"{'date':<12}{'trade':<28}{'hit':<24}{'worst':<14}action")
    for e in events:
        print(f"{e['date']:<12}{e['trade']:<28}{e['hit']:<24}{e['worst']:<14}"
              f"{e['action']}")
    print(f"\nvetoes: {'; '.join(VETOES)}")
