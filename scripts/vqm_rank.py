...
def main() -> int:
    snap = json.load(open("artifacts/fundamentals_snapshot.json"))
    ...
    pillars = {
        "VALUE": [rank("forwardPE", False), rank("priceToBook", False),
                  rank("enterpriseToEbitda", False), rank("fcf_yield", True)],
        "QUALITY": [rank("returnOnEquity", True), rank("returnOnAssets", True),
                    rank("operatingMargins", True), rank("profitMargins", True),
                    rank("debtToEquity", False), rank("currentRatio", True)],
    ...
    cols = ["shortName", "COMPOSITE", "VALUE", "QUALITY", "GROWTH", "MOMENTUM",
            "SENTIMENT", "forwardPE", "fcf_yield", "returnOnEquity",
            "revenueGrowth", "52WeekChange", "upside", "beta", "flags"]
    ...
    payload = {
        "as_of": snap["as_of"],
        "top30": ranked.head(30).round(4).reset_index()
                       .rename(columns={"index": "symbol"}).to_dict("records"),
        "bottom20": ranked.tail(20).round(4).reset_index()
                          .rename(columns={"index": "symbol"}).to_dict("records"),
    }
    atomic_write_bytes(Path("artifacts") / "vqm_rank.json",
