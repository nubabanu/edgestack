...
def main() -> int:
    ...
    for base, name in ((1.0, "overlay_spy_base10.json"), (1.3, "overlay_spy_base13.json")):
        applied, _ = build_exposure(df, base=base)
        payload = {
            "base": base,
            "bars": bars,
            "applied_exposure": [
                {"date": str(d.date()), "L": float(v)} for d, v in applied.items()
            ],
        }
        atomic_write_bytes(FIXTURES / name, json.dumps(payload).encode())
        print(f"{name}: {len(bars)} bars, {len(applied)} exposures, mean L {applied.mean():.3f}")
    ...
    bars_df = bars_df.assign(date=bars_df["dt"].dt.tz_localize(None).dt.normalize()).set_index(
        "date"
    )
    ens = ensemble_exposure(bars_df)
    fams = family_positions(bars_df)
    atomic_write_bytes(
        FIXTURES / "ensemble_spy.json",
        json.dumps(
            {
                "bars": bars,
                "exposure": [{"date": str(d.date()), "e": float(v)} for d, v in ens.items()],
    ...
    r = session.get(
        ...
        params={
            "period1": int(time.time()) - 60 * 86400,
    ...
    res["indicators"]["adjclose"][0]["adjclose"] = res["indicators"]["adjclose"][0]["adjclose"][:n]
    quote["close"][5] = None  # pin drop-null-bars behavior
    quote["open"][5] = None
    atomic_write_bytes(FIXTURES / "yahoo_chart_sample.json", json.dumps(raw).encode())
