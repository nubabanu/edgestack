...
def main() -> int:
    ...
    bars = pd.DataFrame({"close": index, "adj": index,
                         "open": index, "high": index, "low": index})
    ret = ff["mkt"]
    print(f"Ken French daily market factor: {ff.index[0].date()} -> "
    ...
    tde = pd.Series(bars.groupby(ym).cumcount(ascending=False) + 1,
                    index=bars.index)
    tom = (tdom <= 3) | (tde == 1)
    ...
    for era, (lo, hi) in ERAS.items():
        ...
        results[era] = era_row
        print(f"{era:<26}{era_row['ens_sharpe']:>8.2f}{era_row['bh_sharpe']:>8.2f}"
    ...
    pooled = {"alpha_ann": round(nw["alpha_ann"], 4),
              "alpha_t": round(nw["alpha_t"], 2), "beta": round(nw["beta"], 2),
              "eras_sharpe_ge_bh": sum(
                  1 for r in results.values()
                  if r["ens_sharpe"] >= r["bh_sharpe"])}
    ...
    atomic_write_bytes(Path("artifacts") / "french_deep_history.json",
                       json.dumps({"eras": results, "pooled": pooled},
                                  indent=1).encode())
