...
def stage_prep(cfg, catalog) -> None:
    ...
    labels = forward_return_labels(
        panel, (HORIZON,), benchmark_symbol="SPY", roundtrip_cost=roundtrip
    )
    ...
    print(
        f"rules frame: {len(merged):,} eligible rows -> {MERGED_PATH} "
...
def _pipeline_on(pre: pd.DataFrame, cfg, seed: int, label: str) -> dict:
    ...
    print(
        f"  [{label}] EBM shortlist: {len(shortlist)}/{len(feature_cols)} features "
        f"({time.time() - t0:.0f}s)"
    )
    ...
    passing = [r for r in rules if r.passes and r.direction > 0 and r.inner_abs_mean > 0]
    print(
        f"  [{label}] structural rules: {len(rules)}, PASSING long: "
...
def _rule_row(r: StableRule) -> dict:
    return {
        ...
        "freq": round(r.selection_freq, 3),
        "sign_cons": round(r.sign_consistency, 3),
        "inner_n": r.inner_n,
        "inner_mean_excess": round(r.inner_mean_excess, 5),
...
def _indicator_frame(frame: pd.DataFrame, rules: list[StableRule]) -> pd.DataFrame:
    feats = frame[
        [
            ...
            if c
            not in ("symbol", "date", "label_end", "gross_ret", "excess_net_ret", "mae", "close")
        ]
    ].astype(np.float64)
    feats = feats.fillna(feats.median(numeric_only=True))
    out = {}
    for r in rules:
        preds = (r.condition,) if isinstance(r.condition, Predicate) else r.condition.conditions
...
def stage_year(cfg, catalog, year: int, phase: str) -> None:
    ...
    if phase == "discover":
        pre = merged.loc[(merged["date"] < cutoff) & merged["excess_net_ret"].notna()].reset_index(
            drop=True
        )
        print(f"=== RULES DISCOVER {year}: {len(pre):,} pre rows ===")
        experiment_id = catalog.record_experiment(
            "rules_outer", end_date=date(year, 12, 31), notes=f"rules campaign outer={year}"
        )
    ...
    print(
        f"  distiller={row['policy']['distiller']}, passing rules={len(passing)}, "
        f"signals={len(intents)}"
    )
    print(
        f"  outer: return {row['strategy_return']:+.1%} "
...
def _persist(year: int, row: dict, rets: pd.Series, trades: pd.DataFrame) -> None:
    ...
    atomic_write_bytes(
        WINDOWS_DIR / f"{year}.json", json.dumps(clean, indent=2, default=str).encode()
    )
...
def stage_report(cfg, catalog) -> None:
    ...
    atomic_write_bytes(
        Path(cfg.paths.artifacts_dir) / "rules_campaign.json",
...
def _verdict(report: dict) -> dict:
    ...
    checks["stable_rules_found_every_window"] = (
        all(w.get("n_passing", 0) > 0 for w in windows) if windows else False
    )
    pos = [w for w in traded if (w.get("strategy_return") or 0) > 0]
    checks["positive_most_outer_windows"] = len(pos) > len(traded) / 2 if traded else False
    ...
    checks["not_dependent_on_one_stock"] = (
        bool(conc and conc.get("best_symbol_share") is not None and conc["best_symbol_share"] < 0.5)
        if conc
        else None
    )
    beats = [
...
def run_canary(*, with_effect: bool, seed: int = 42) -> dict:
    ...
    cfg = EdgeStackConfig.model_validate(
        {
            "universe": {"symbols": ["AAA", "BBB", "CCC", "DDD", "EEE"], "benchmark_symbol": "AAA"},
            "validation": {"final_test_start": "2026-01-01"},
        }
    )
    effects = (
        (CalendarEffect(facts_column="is_turn_of_month", drift_bps=80.0),) if with_effect else ()
    )
    ...
    features = build_features(panel, cfg, specs)
    roundtrip = 0.001
    labels = forward_return_labels(panel, (5,), benchmark_symbol="AAA", roundtrip_cost=roundtrip)
    merged = features.merge(
        labels[["symbol", "date", "label_end", "gross_ret", "excess_net_ret"]],
        on=["symbol", "date"],
        how="inner",
    ).dropna(subset=["excess_net_ret"])
...
def main() -> int:
    ...
    parser.add_argument("--stage", choices=["prep", "year", "report", "canary"], required=True)
