...
def _edge(edge_id: str, family: Family, net: float = 0.005, horizon: int = 10) -> Edge:
    return Edge(
        ...
        stats=EdgeStats(
            ...
            gross_mean_return=net + 0.002,
            net_mean_return=net,
            median_return=net,
            return_std=0.02,
            downside_deviation=0.01,
            probability_of_profit=0.56,
            probability_of_positive_net_return=0.55,
...
def test_correlated_family_cannot_stack_conviction() -> None:
    ...
    five_same = aggregate_evidence([_edge(f"m{i}", Family.MOMENTUM) for i in range(5)], scoring)
    families = [Family.MOMENTUM, Family.TREND, Family.CALENDAR, Family.VOLUME, Family.STRUCTURE]
...
def test_entry_plan_next_open_and_zone() -> None:
    ...
    plan = build_entry_plan(Side.LONG, close=100.0, atr=2.0, as_of=date(2023, 4, 6), calendar=cal)
