...
def _bar(open_=100.0, high=105.0, low=95.0, close=102.0, volume=1e6) -> Bar:
    return Bar(session=SESSION, open=open_, high=high, low=low, close=close, volume=volume)


def _order(qty: float, order_type: OrderType, **kw) -> Order:
    return Order(symbol="TST", quantity=qty, order_type=order_type, created_session=SESSION, **kw)
...
def test_market_on_open_and_close_phases() -> None:
    ...
    assert sim.try_fill(_order(100, OrderType.MARKET_ON_OPEN), _bar(), at_open_phase=False) is None
    moc = sim.try_fill(_order(100, OrderType.MARKET_ON_CLOSE), _bar(), at_open_phase=False)
...
def test_buy_limit_rules(open_: float, low: float, limit: float, expected: float | None) -> None:
    fill = _sim().try_fill(
        _order(100, OrderType.LIMIT, limit_price=limit),
        _bar(open_=open_, low=low),
        at_open_phase=False,
    )
    if expected is None:
        assert fill is None
...
def test_sell_stop_rules_including_gap(
    open_: float, low: float, stop: float, expected: float | None
) -> None:
    fill = _sim().try_fill(
        _order(-100, OrderType.STOP, stop_price=stop),
        _bar(open_=open_, low=low, close=max(low, 95.0)),
        at_open_phase=False,
    )
    if expected is None:
        assert fill is None
...
def test_buy_stop_gap_through_fills_at_open() -> None:
    ...
    fill = _sim().try_fill(
        ...
        _bar(open_=108.0, high=110.0, low=107.0, close=109.0),
        at_open_phase=False,
    )
...
def test_participation_cap_creates_partial_fill() -> None:
    fill = _sim().try_fill(
        _order(1_000_000, OrderType.MARKET_ON_OPEN),
        _bar(volume=1e6),
        at_open_phase=True,
    )
