...
def _req(
    symbol: str, side: Side = Side.LONG, conviction: float = 70.0, vol: float = 0.25
) -> AllocationRequest:
    return AllocationRequest(symbol=symbol, side=side, conviction=conviction, annualized_vol=vol)


def test_equal_risk_prefers_low_vol() -> None:
    risk = RiskConfig(max_position_weight=0.5, max_gross_exposure=1.0)
    weights = allocate([_req("CALM", vol=0.10), _req("WILD", vol=0.50)], risk, method="equal_risk")
...
def test_score_weighted_ignores_sub_neutral_conviction() -> None:
    risk = RiskConfig()
    weights = allocate(
        [_req("GOOD", conviction=80), _req("MEH", conviction=50)],
        risk,
...
def test_vol_target_scales_gross_to_target() -> None:
    ...
    weights = allocate([_req("A", vol=0.20), _req("B", vol=0.20)], risk, method="vol_target")
