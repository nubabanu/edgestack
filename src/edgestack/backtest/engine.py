...
class BacktestEngine:
    def __init__(self, panel: pd.DataFrame, cfg: EdgeStackConfig,
                 scenario: CostScenario | None = None) -> None:
        self.cfg = cfg
        ...
        for row in panel.itertuples(index=False):
            bar = Bar(session=row.date, open=row.open, high=row.high,
                      low=row.low, close=row.close, volume=row.volume)
            self.bars.setdefault(row.date, {})[row.symbol] = bar

    def run(self, intents: list[TradeIntent],
            initial_cash: float = 100_000.0) -> Ledger:
        ...
        for pos_idx, session in enumerate(self.sessions):
            ...
            self._process_exits(session, pos_idx, day_bars, portfolio,
                                open_trades, ledger)
            ...
            ledger.record_equity(session, portfolio.cash, equity,
                                 portfolio.gross_exposure(last_prices),
                                 len(portfolio.open_positions()), borrow)
    ...
    def _process_exits(self, session: pd.Timestamp, pos_idx: int,
                       day_bars: dict[str, Bar], portfolio: PortfolioState,
                       open_trades: dict[str, _OpenTrade], ledger: Ledger) -> None:
        for symbol in list(open_trades):
            ...
            if pos_idx >= trade.exit_session_pos:
                order = Order(symbol=symbol, quantity=qty,
                              ...
                              created_session=session, tag="time_exit")
                exit_fill = self.simulator.try_fill(order, bar, at_open_phase=True)
                reason = "time_exit"

            if exit_fill is None:
                stop_order = Order(symbol=symbol, quantity=qty,
                                   order_type=OrderType.STOP,
                                   created_session=session,
                                   stop_price=trade.stop_price, tag="stop_loss")
                exit_fill = self.simulator.try_fill(stop_order, bar, at_open_phase=False)
                reason = "stop_loss"

            if exit_fill is None:
                target_order = Order(symbol=symbol, quantity=qty,
                                     order_type=OrderType.LIMIT,
                                     created_session=session,
                                     limit_price=trade.target_price, tag="target")
            ...
            if exit_fill is not None:
                ...
                net_pnl = (
                    direction * abs(trade.quantity)
                    * (exit_fill.price - trade.entry_price)
                    - exit_fill.cost - trade.entry_cost
                )
                ledger.record_trade(
                    symbol=symbol, side=trade.side.value,
                    entry_session=trade.entry_session or session,
                    exit_session=session, entry_price=trade.entry_price,
                    exit_price=exit_fill.price, quantity=trade.quantity,
                    net_pnl=net_pnl, exit_reason=reason,
                )
                del open_trades[symbol]

    def _process_entries(self, session: pd.Timestamp, day_bars: dict[str, Bar],
                         portfolio: PortfolioState,
                         open_trades: dict[str, _OpenTrade],
                         pending: list[tuple[Order, TradeIntent]],
                         ledger: Ledger) -> list[tuple[Order, TradeIntent]]:
        ...
        for order, intent in pending:
            ...
            open_trades[intent.symbol] = _OpenTrade(
                side=intent.side, stop_price=intent.stop_price,
                target_price=intent.target_price, exit_session_pos=exit_pos,
                entry_price=fill.price, entry_session=session,
                quantity=fill.quantity, entry_cost=fill.cost,
            )
    ...
    def _entry_order(self, intent: TradeIntent, session: pd.Timestamp,
        ...
        return Order(symbol=intent.symbol, quantity=signed,
                     order_type=OrderType.MARKET_ON_OPEN,
                     created_session=session, tag="entry")

    def _force_close_remaining(self, portfolio: PortfolioState,
                               open_trades: dict[str, _OpenTrade],
                               last_prices: dict[str, float],
                               ledger: Ledger) -> None:
        ...
        for symbol in list(open_trades):
            ...
            if bar is None:
                continue
            order = Order(symbol=symbol, quantity=-trade.quantity,
                          order_type=OrderType.MARKET_ON_CLOSE,
                          created_session=last_session, tag="end_of_data")
            ...
            ledger.record_fill(fill)
            direction = 1.0 if trade.side is Side.LONG else -1.0
            net_pnl = (direction * abs(trade.quantity)
                       * (fill.price - trade.entry_price)
                       - fill.cost - trade.entry_cost)
            ledger.record_trade(
                symbol=symbol, side=trade.side.value,
                entry_session=trade.entry_session or last_session,
                exit_session=last_session, entry_price=trade.entry_price,
                exit_price=fill.price, quantity=trade.quantity,
                net_pnl=net_pnl, exit_reason="end_of_data",
            )
            del open_trades[symbol]
